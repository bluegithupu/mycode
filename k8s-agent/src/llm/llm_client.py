...
class LLMClient:
    def __init__(self):
        ...
        self.api_key = "sk-l3WytTQtQnrtrXINA22c07C3Ba694d87A3F0B218Bc7509F1"
    ...
    def analyze_command(self, user_input: str) -> Dict:
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                        "content": "你是一个Kubernetes运维专家，负责生成安全的kubectl命令。直接返回JSON对象，不要使用markdown格式。"},
            ...
            try:
                ...
                raise Exception(f"LLM返回的响应不是有效的JSON格式: {content}")

        except Exception as e:
            raise Exception(f"LLM API调用失败: {str(e)}")

    def analyze_result(self, command: str, result: str) -> Dict:
        """分析命令执行结果"""
        prompt = f"""
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                        "content": "你是一个Kubernetes运维专家，负责分析命令执行结果。直接返回JSON对象，不要使用markdown格式。"},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=self.max_tokens,
                temperature=self.temperature
